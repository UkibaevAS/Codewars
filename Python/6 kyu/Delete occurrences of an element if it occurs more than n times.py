...
to show Charlie their entire collection. However, Charlie doesn't like these sessions, since the motif usually repeats.
...
def delete_nth(order,max_e):
    ...
    result = []
    for num in order:
        if num not in control_dict.keys():
            control_dict[num] = 1
        else:
            control_dict[num] += 1
        if control_dict.get(num) <= max_e:
            result.append(num)
    return result